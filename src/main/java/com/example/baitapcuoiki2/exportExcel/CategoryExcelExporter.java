...
public class CategoryExcelExporter {
    public static ByteArrayInputStream exportCategoryToExcel(List<CategoryExportResponse> data) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            // Create data rows
            int rowIdx = 1;
            for (CategoryExportResponse category : data) {
                Row row = sheet.createRow(rowIdx++);
                createCell(row, 0, category.getId(), dataStyle);
                createCell(row, 1, category.getCategoryName(), dataStyle);
                createCell(row, 2, category.getCategoryCode(), dataStyle);
                createCell(row, 3, category.getDescription(), dataStyle);
                createCell(row, 4, category.getCreatedDate() != null ? new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(category.getCreatedDate()) : "", dateStyle);
                createCell(row, 5, category.getModifiedDate() != null ? new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(category.getModifiedDate()) : "", dateStyle);
                createCell(row, 6, category.getCreatedBy(), dataStyle);
                createCell(row, 7, category.getModifiedBy(), dataStyle);
            }
